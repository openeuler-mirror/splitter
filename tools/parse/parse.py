...
def write_files(pkg_path: str, output_dir: str, matched_files: List[str]):
    ...
    try:
        ...
        logger.info(f"Files have been successfully extracted to: {output_dir}")
    except subprocess.CalledProcessError as e:
        raise RuntimeError(f"Failed to extract files from RPM '{pkg_path}': {e}") from e
    finally:
...
def extract_files(pkg_path: str, output_dir: str, patterns: List[str]):
    ...
    logger.info(f"Package matched files: \n{json.dumps(matched_files, indent=4)}")
